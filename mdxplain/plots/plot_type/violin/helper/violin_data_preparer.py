...
class ViolinDataPreparer:
    ...
    def prepare_from_feature_importance(
        ...
        features, optionally converts contacts to distances, prepares violin data,
        ...
        >>> violin_data, metadata, colors, cutoff = ViolinDataPreparer.prepare_from_feature_importance(
        ...
        ... )
        >>> print(violin_data.keys())  # ["contacts", "torsions"]
        >>> print(cutoff)  # None
        ...
        # Build violin data structure
        violin_data = ViolinDataPreparer._build_violin_data(
            pipeline_data,
            continuous_selector,
            comparison_data_selector_names,
            feature_map,
        ...
        feature_metadata_map = ViolinDataPreparer._collect_feature_metadata(
            violin_data, metadata_array
        )
        ...
        return violin_data, feature_metadata_map, data_selector_colors, contact_cutoff
    ...
    def prepare_from_manual_selection(
        ...
        features from selector, builds violin data, collects feature metadata,
        ...
        --------
        >>> # With contact transformation (default)
        >>> violin_data, metadata, colors, cutoff = ViolinDataPreparer.prepare_from_manual_selection(
    ...
    def _build_violin_data(
        ...
        """
        Build violin data structure from features.
        ...
        Examples
        --------
        >>> # Build violin data from features
    ...
    def _collect_feature_metadata(
        ...
        """
        Collect feature metadata for all features in violin data.
        ...
        ----------
        violin_data : Dict[str, Dict[str, Dict[str, np.ndarray]]]
            Violin data structure (feat_type -> feat_name -> selector -> values)
        metadata_array : np.ndarray
        ...
        Examples
        --------
        >>> metadata_map = ViolinDataPreparer._collect_feature_metadata(
        ...     pipeline_data, "selector", violin_data, metadata_array
        ... )
        >>> dssp_meta = metadata_map["dssp"]["LYS42_DSSP_H"]
        >>> print(dssp_meta["feature_info"]["dssp_class"])  # "H"

        Notes
        -----
        For each feature, stores:
        - type_metadata: Full metadata from feature type (encoding, classes, etc.)
        - features: Specific metadata for this feature (from features array)
        """
        result = {}

        # For each feature type in violin data
        for feat_type in violin_data.keys():
            result[feat_type] = ViolinDataPreparer._collect_metadata_for_type(
                feat_type, violin_data[feat_type], metadata_array
            )

        return result

    @staticmethod
    def _collect_metadata_for_type(
        feat_type: str,
        features: Dict[str, Dict[str, np.ndarray]],
        metadata_array: np.ndarray
    ) -> Dict[str, Dict[str, Any]]:
        """
        Collect metadata for all features of a specific type.
        ...
        Examples
        --------
        >>> meta = ViolinDataPreparer._collect_metadata_for_type(
        ...     "dssp", features, metadata_array
        ... )
        """
        result = {}
        top_level_metadata = ViolinDataPreparer._get_top_level_metadata(
            feat_type, metadata_array
        )

        for feat_name in features.keys():
            feature_info = ViolinDataPreparer._get_feature_info(
    ...
    @staticmethod
    def _get_top_level_metadata(
        feat_type: str, metadata_array: np.ndarray
    ) -> Dict[str, Any]:
        """
        Get top-level metadata for a feature type from metadata array.
        ...
        -------
        Dict[str, Any]
            Top-level metadata (encoding, classes, etc.)

        Examples
        --------
        >>> top_level = ViolinDataPreparer._get_top_level_metadata(
        ...     "dssp", metadata_array
        ... )
    ...
    def _get_feature_info(
        feat_name: str, metadata_array: np.ndarray
    ) -> Dict[str, Any]:
        """
        ...
        Examples
        --------
        >>> info = ViolinDataPreparer._get_feature_info(
        ...
        for idx in range(len(metadata_array)):
            if FeatureMetadataUtils.get_feature_name(metadata_array, idx) == feat_name:
                return ViolinDataPreparer._extract_feature_info_from_entry(
                    metadata_array[idx]
                )
        return {}

    @staticmethod
    def _extract_feature_info_from_entry(meta_entry: Dict[str, Any]) -> list:
        """
        Extract feature info from metadata entry.

        Returns the full features list for consistency across all feature
        types. For pair features (distances), this preserves both partners.
        ...
        >>> info = ViolinDataPreparer._extract_feature_info_from_entry(entry)
        >>> print(info)  # [{"dssp_class": "H"}]

        >>> # Pair feature (distances)
        >>> entry = {"features": [{"partner": 1}, {"partner": 2}], "type": "distances"}
        >>> info = ViolinDataPreparer._extract_feature_info_from_entry(entry)
