...
class TorsionsSelectionService:
    """
    ...
    def __call__(
        ...
        self._manager.add_selection(
            self._pipeline_data, selector_name, "torsions", selection,
    ...
    def _add_reduction_config(self, selector_name: str, metric: str, threshold_min: Optional[float], threshold_max: Optional[float], cross_trajectory: bool = True, extra_params: Optional[dict] = None) -> None:
        ...
        in the most recently added selection of the torsions feature type.
        ...
        selector_name : str
            Name of the feature selector configuration
        metric : str
            Statistical metric for reduction (e.g., 'transitions', 'std', 'max')
        threshold_min : float, optional
        ...
        --------
        None
            Modifies the last torsions selection in-place
        """
        selector_data = self._pipeline_data.selected_feature_data[selector_name]
        if "torsions" not in selector_data.selections:
            raise ValueError(f"No torsions selections for selector '{selector_name}'")
        last_selection = selector_data.selections["torsions"][-1]
