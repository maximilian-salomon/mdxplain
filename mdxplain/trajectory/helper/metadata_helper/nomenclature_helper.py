...
class NomenclatureHelper:
    ...
    def _create_structured_labels(self) -> List[Dict]:
        ...
        for idx, residue in enumerate(self.topology.residues):
            ...
            full_name = self._get_full_name(aaa_code, a_code, residue.resSeq, consensus)

            label_dict = {
                "aaa_code": aaa_code,
                "a_code": a_code,
                "index": residue.index,
                "seqid": residue.resSeq,
                "consensus": consensus,
                "full_name": full_name,
            }
    ...
    def _get_full_name(
        self, aaa_code: str, a_code: str, residue_seqid: int, consensus: Optional[str]
    ) -> str:
        """
        ...
        residue_seqid : int
            Residue sequence ID from topology (resSeq)
        ...
        base_name = f"{base_code}{residue_seqid}"
