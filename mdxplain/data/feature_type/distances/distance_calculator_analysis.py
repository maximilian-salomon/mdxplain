import numpy as np
from ..helper.CalculatorStatHelper import CalculatorStatHelper
...
class DistanceCalculatorAnalysis:
    ...
    def compute_q25(self, distances):
        ...
        return CalculatorStatHelper.compute_func_per_pair(distances, lambda x, axis: np.percentile(x, 25, axis=axis), self.chunk_size)
    
    def compute_q75(self, distances):
        """Compute 75th percentile of distances per pair."""
        return CalculatorStatHelper.compute_func_per_pair(distances, lambda x, axis: np.percentile(x, 75, axis=axis), self.chunk_size)
    
    def compute_iqr(self, distances):
        """Compute interquartile range of distances per pair."""
        return CalculatorStatHelper.compute_func_per_pair(distances, lambda x, axis: np.percentile(x, 75, axis=axis) - np.percentile(x, 25, axis=axis), self.chunk_size)
    
    def compute_mad(self, distances):
        """Compute median absolute deviation of distances per pair."""
        return CalculatorStatHelper.compute_func_per_pair(distances, lambda x, axis: np.median(np.abs(x - np.median(x, axis=axis, keepdims=True)), axis=axis), self.chunk_size)
    ...
    def compute_transitions_lagtime(self, distances, threshold=2.0, lag_time=10):
        ...
        return CalculatorStatHelper.compute_transitions_within_lagtime(distances, threshold, lag_time, self.chunk_size)
    
    def compute_transitions_window(self, distances, threshold=2.0, window_size=10):
        """Compute transitions within window."""
        return CalculatorStatHelper.compute_transitions_within_window(distances, threshold, window_size, self.chunk_size)
    
    def compute_stability(self, distances, threshold=2.0, window_size=10, mode='window'):
        """Compute stability analysis."""
        return CalculatorStatHelper.compute_stability(distances, threshold, window_size, self.chunk_size, mode)
    ...
    def compute_differences(self, distances1, distances2, preprocessing_func=None):
        ...
        return CalculatorStatHelper.compute_differences(distances1, distances2, self.chunk_size, preprocessing_func) 