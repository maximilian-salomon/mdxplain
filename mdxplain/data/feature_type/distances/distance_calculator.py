...
from .DistanceCalculatorAnalysis import DistanceCalculatorAnalysis
...
class DistanceCalculator(CalculatorBase):
    ...
    def compute(self, trajectories, ref=None, squareform=False, k=0):
        ...
        ref, total_frames, distances, condensed_path = self._setup_computation(trajectories, ref, squareform, k)
        ...
        distances = self._finalize_output(distances, res_list, total_frames, condensed_path, squareform)
    ...
    def _compute_metric_values(self, distances, metric, threshold, window_size, 
        ...
        if metric == 'cv':
            ...
            if transition_mode == 'window':
                return self.analysis.compute_transitions_window(distances, threshold=threshold, window_size=window_size)
            elif transition_mode == 'lagtime':
                return self.analysis.compute_transitions_lagtime(distances, threshold=threshold, lag_time=lag_time)
            else:
    ...
    def compute_dynamic_values(self, input_data, metric='cv', threshold_min=None, threshold_max=None, 
