...
class ContactCalculatorAnalysis:
    ...
    def compute_transitions_lagtime(self, contacts, threshold=1, lag_time=1):
        ...
        return CalculatorStatHelper.compute_transitions_within_lagtime(contacts, threshold, lag_time, self.chunk_size)
    
    def compute_transitions_window(self, contacts, threshold=1, window_size=10):
        """Compute transitions within window."""
        return CalculatorStatHelper.compute_transitions_within_window(contacts, threshold, window_size, self.chunk_size)
    
    def compute_stability(self, contacts, threshold=1, window_size=1):
        """Compute stability analysis."""
        return CalculatorStatHelper.compute_stability(contacts, threshold, window_size, self.chunk_size)
    
    # === COMPARISON METHODS ===
    def compute_differences(self, contacts1, contacts2, preprocessing_func=None):
        """Compute differences between two contact datasets."""
        return CalculatorStatHelper.compute_differences(contacts1, contacts2, self.chunk_size, preprocessing_func) 