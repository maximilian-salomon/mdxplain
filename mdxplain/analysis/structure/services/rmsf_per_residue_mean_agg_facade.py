...
class RMSFPerResidueMeanAggFacade(BaseRMSFPerResidueAggFacade):
    """Expose per-residue RMSF service with mean aggregation.

    Provides access to :class:`RMSFPerResidueService` configured with mean
    ...
    def __init__(
        ...
        aggregation type is hard-coded to mean for this facade.
        ...
        if pipeline_data is None:
            raise ValueError("RMSFPerResidueMeanAggFacade requires pipeline_data")
    ...
    def service(self) -> RMSFPerResidueService:
        ...
        Provides the residue service that averages atom-level RMSF values via
        the arithmetic mean with the configured deviation metric.
