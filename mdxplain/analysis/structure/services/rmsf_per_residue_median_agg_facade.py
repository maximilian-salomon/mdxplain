...
class RMSFPerResidueMedianAggFacade(BaseRMSFPerResidueAggFacade):
    """Expose per-residue RMSF service with median aggregation.

    Provides access to :class:`RMSFPerResidueService` configured with median
    ...
    def __init__(
        ...
        aggregation type is hard-coded to median for this facade.
        ...
        if pipeline_data is None:
            raise ValueError("RMSFPerResidueMedianAggFacade requires pipeline_data")
    ...
    def service(self) -> RMSFPerResidueService:
        ...
        Provides the residue service that aggregates atom-level RMSF values via
        the statistical median with the configured deviation metric.
