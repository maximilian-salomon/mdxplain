...
class RMSDMadService:
    ...
    computation using the median absolute deviation (MAD) metric for
    outlier-resistant RMSD values.

    Examples
    --------
    >>> service = RMSDMadService(pipeline_data)  # doctest: +SKIP
    ...
    def __init__(self, pipeline_data: PipelineData | None) -> None:
        ...
        if pipeline_data is None:
            raise ValueError("RMSDMadService requires pipeline_data")
    ...
    def to_reference(
        ...
        delegates the numerical work to the calculator using the median
        absolute deviation (MAD) metric for maximum outlier resistance.
    ...
    def frame_to_frame(
        ...
        RMSD values between frames separated by ``lag`` using the MAD metric.
    ...
    def window_frame_to_start(
        ...
        first frame and condensed via the MAD metric into a single RMSD value
        per window.
    ...
    def window_frame_to_frame(
        ...
        condensed using the MAD metric.
