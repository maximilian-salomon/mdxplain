...
class CalculatorComputeHelper:
    ...
    @staticmethod
    def compute_dynamic_values(data, metric_values, metric_name,
                             threshold_min=None, threshold_max=None,
                             feature_names=None, use_memmap=False, output_path=None, chunk_size=1000):
        """
        ...
        if n_selected == 0:
            ...
            warnings.warn(f"No values found within the specified threshold criteria. "
    ...
    def _handle_feature_names(feature_names, mask):
        ...
        if feature_names is not None:
            if len(feature_names) != mask.size:
                warnings.warn(f"feature_names length ({len(feature_names)}) doesn't match "
    ...
    def _extract_dynamic_data(data, mask, use_memmap, output_path, chunk_size):
        ...
        if use_memmap:
            ...
            n_selected = np.sum(mask.flatten())
            dynamic_data = np.memmap(output_path, dtype=data.dtype, mode='w+',
    ...
    def calculate_output_dimensions(input_shape, squareform, k):
        ...
        needs_conversion = (len(input_shape) == 3 and not squareform) or (len(input_shape) == 2 and squareform)
